/**
 * TTLLM reference ttlink Worker — exact substring search over embedded free-core demo corpus.
 * Down to the bone: real matches, real hashes, rate limit headers. Not multi-trillion.
 */
  ...
  for (const [docId, doc] of Object.entries(INDEX)) {
    ...
    while (true) {
      ...
      hits.push({
        ...
        context: text.slice(a, b),
        doc_sha256: doc.h,
        match: text.slice(i, i + span.length),
      });
...
export default {
  async fetch(request) {
    ...
    if (url.pathname === "/health") {
      return Response.json({ ok: true, service: "ttlink-worker", docs: Object.keys(INDEX).length });
    }
    if (url.pathname === "/v1/ttlink/stats") {
      return Response.json({
        documents: Object.keys(INDEX).length,
        note: "reference demo corpus only",
      });
    }
    if (url.pathname === "/v1/ttlink/query" && request.method === "POST") {
      if (!allow(ip)) {
      ...
      return Response.json(
        { hits, count: hits.length, honesty: "exact match on embedded free-core demo corpus" },
        { headers: { "Access-Control-Allow-Origin": "*" } },
      );
    }
    return Response.json({
      service: "ttllm-ttlink-reference",
      endpoints: ["/health", "/v1/ttlink/stats", "POST /v1/ttlink/query"],
