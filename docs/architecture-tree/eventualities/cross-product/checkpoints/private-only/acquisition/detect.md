# checkpoints · private-only · acquisition · detect

**Path:** `eventualities/cross-product/checkpoints/private-only/acquisition/detect.md`  
**Updated:** 2026-08-08  
**Leaf type:** detect — how we notice this failure mode early

## Purpose
How we notice this failure mode early for: **checkpoints** · material that should be free-core is held private without BOUNDARY justification · phase under change-of-control or acquisition pressure · actor project lead / founder.
...
3. Compare published claims (site, README, scorecard) to artefacts under `models/*/checkpoints/step_*.pt`.
...
## Output of detection
Write a one-paragraph incident/note with: path `cross-product/checkpoints/private-only/acquisition/detect.md`, evidence command output, severity guess (Low–Critical per Domain 5 thresholds).
