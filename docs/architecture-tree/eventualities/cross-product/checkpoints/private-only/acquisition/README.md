# checkpoints · private-only · acquisition

**Path:** `eventualities/cross-product/checkpoints/private-only/acquisition/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone / free public core / product is the proof

## What this eventuality is
The **intermediate training checkpoints** is compromised by: **material that should be free-core is held private without BOUNDARY justification**.  
Lifecycle phase: **under change-of-control or acquisition pressure**.  
Primary actor context: **any actor (founder default)**.

## Why it matters
Open weights or polished demos without verifiable intermediate training checkpoints are soft tissue.  
This path exists so the failure mode cannot hide in tribal knowledge.

## Where the bone lives
| Item | Location |
|------|----------|
| Artefacts | `models/*/checkpoints/step_*.pt` |
| Domain / policy | Epoch 02 release discipline |
| Verify | `CHECKPOINTS_MANIFEST` |
| Register on failure | registers/incidents/ + scorecard |
...
3. Phase-specific (under change-of-control or acquisition pressure): pre-release CI/manual gate; release seal script; post-release monitor; incident response; acquisition due-diligence freeze.
...
1. **Stop the soft tissue:** pause any public claim that depends on healthy intermediate training checkpoints.
...
- BOUNDARY: commercial features cannot require opacity of intermediate training checkpoints if it is free-core.
