# stream · private-only · release · detect

**Path:** `eventualities/cross-product/stream/private-only/release/detect.md`  
**Updated:** 2026-08-08  
**Leaf type:** detect — how we notice this failure mode early

## Purpose
How we notice this failure mode early for: **stream** · material that should be free-core is held private without BOUNDARY justification · phase at seal/publish time · actor project lead / founder.
...
## Output of detection
Write a one-paragraph incident/note with: path `cross-product/stream/private-only/release/detect.md`, evidence command output, severity guess (Low–Critical per Domain 5 thresholds).
