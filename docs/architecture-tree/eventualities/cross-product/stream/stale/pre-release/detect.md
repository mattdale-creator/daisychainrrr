# stream · stale · pre-release · detect

**Path:** `eventualities/cross-product/stream/stale/pre-release/detect.md`  
**Updated:** 2026-08-08  
**Leaf type:** detect — how we notice this failure mode early

## Purpose
How we notice this failure mode early for: **stream** · artefact exists but is outdated relative to current release · phase before public release or claim · actor project lead / founder.
...
## Output of detection
Write a one-paragraph incident/note with: path `cross-product/stream/stale/pre-release/detect.md`, evidence command output, severity guess (Low–Critical per Domain 5 thresholds).
