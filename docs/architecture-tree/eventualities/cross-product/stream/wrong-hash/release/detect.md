# stream · wrong-hash · release · detect

**Path:** `eventualities/cross-product/stream/wrong-hash/release/detect.md`  
**Updated:** 2026-08-08  
**Leaf type:** detect — how we notice this failure mode early

## Purpose
How we notice this failure mode early for: **stream** · published sha256/Merkle root does not match re-hash of files · phase at seal/publish time · actor project lead / founder.
...
## Output of detection
Write a one-paragraph incident/note with: path `cross-product/stream/wrong-hash/release/detect.md`, evidence command output, severity guess (Low–Critical per Domain 5 thresholds).
