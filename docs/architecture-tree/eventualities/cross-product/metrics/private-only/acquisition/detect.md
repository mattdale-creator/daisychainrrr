# metrics · private-only · acquisition · detect

**Path:** `eventualities/cross-product/metrics/private-only/acquisition/detect.md`  
**Updated:** 2026-08-08  
**Leaf type:** detect — how we notice this failure mode early

## Purpose
How we notice this failure mode early for: **metrics** · material that should be free-core is held private without BOUNDARY justification · phase under change-of-control or acquisition pressure · actor project lead / founder.
...
3. Compare published claims (site, README, scorecard) to artefacts under `models/*/metrics/train.jsonl`.
...
## Output of detection
Write a one-paragraph incident/note with: path `cross-product/metrics/private-only/acquisition/detect.md`, evidence command output, severity guess (Low–Critical per Domain 5 thresholds).


## Artefacts / tools
- Paths: `models/*/metrics/train.jsonl`
