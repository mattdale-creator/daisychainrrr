# metrics · private-only · acquisition

**Path:** `eventualities/cross-product/metrics/private-only/acquisition/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone / free public core / product is the proof

## What this eventuality is
The **training metrics / logs** is compromised by: **material that should be free-core is held private without BOUNDARY justification**.  
Lifecycle phase: **under change-of-control or acquisition pressure**.  
Primary actor context: **any actor (founder default)**.

## Why it matters
Open weights or polished demos without verifiable training metrics / logs are soft tissue.  
This path exists so the failure mode cannot hide in tribal knowledge.

## Where the bone lives
| Item | Location |
|------|----------|
| Artefacts | `models/*/metrics/train.jsonl` |
| Domain / policy | Domain 4 evaluation |
| Verify | `hyperparams.json` |
...
1. **Stop the soft tissue:** pause any public claim that depends on healthy training metrics / logs.
...
- BOUNDARY: commercial features cannot require opacity of training metrics / logs if it is free-core.
