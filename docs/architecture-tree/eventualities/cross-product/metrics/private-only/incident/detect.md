# metrics · private-only · incident · detect

**Path:** `eventualities/cross-product/metrics/private-only/incident/detect.md`  
**Updated:** 2026-08-08  
**Leaf type:** detect — how we notice this failure mode early

## Purpose
How we notice this failure mode early for: **metrics** · material that should be free-core is held private without BOUNDARY justification · phase during or after a known incident · actor project lead / founder.
...
## Output of detection
Write a one-paragraph incident/note with: path `cross-product/metrics/private-only/incident/detect.md`, evidence command output, severity guess (Low–Critical per Domain 5 thresholds).
