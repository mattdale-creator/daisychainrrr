# eval · private-only · acquisition

**Path:** `eventualities/cross-product/eval/private-only/acquisition/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone / free public core / product is the proof

## What this eventuality is
The **evaluation artefacts** is compromised by: **material that should be free-core is held private without BOUNDARY justification**.  
Lifecycle phase: **under change-of-control or acquisition pressure**.  
Primary actor context: **any actor (founder default)**.

## Why it matters
Open weights or polished demos without verifiable evaluation artefacts are soft tissue.  
This path exists so the failure mode cannot hide in tribal knowledge.

## Where the bone lives
| Item | Location |
|------|----------|
| Artefacts | `models/*/evals/` |
| Domain / policy | Domain 4 public claim gate |
| Verify | `eval_pack.json` |
| Register on failure | registers/decisions/ if claim involved |
...
1. **Stop the soft tissue:** pause any public claim that depends on healthy evaluation artefacts.
...
- BOUNDARY: commercial features cannot require opacity of evaluation artefacts if it is free-core.
