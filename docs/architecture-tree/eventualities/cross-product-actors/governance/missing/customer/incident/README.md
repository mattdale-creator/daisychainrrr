# governance · missing · customer · incident

**Path:** `eventualities/cross-product-actors/governance/missing/customer/incident/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone / free public core / product is the proof

## What this eventuality is
The **org decision-making** is compromised by: **required artefact is absent when a public claim or release depends on it**.  
Lifecycle phase: **during or after a known incident**.  
Primary actor context: **commercial-layer customer or prospect**.

## Why it matters
Open weights or polished demos without verifiable org decision-making are soft tissue.  
This path exists so the failure mode cannot hide in tribal knowledge.

## Where the bone lives
| Item | Location |
|------|----------|
| Artefacts | `registers/decisions/` |
| Domain / policy | Domain 1 |
| Verify | `LOG.md + feed.json` |
| Register on failure | registers/decisions/ |
...
1. **Stop the soft tissue:** pause any public claim that depends on healthy org decision-making.
...
- BOUNDARY: commercial features cannot require opacity of org decision-making if it is free-core.
