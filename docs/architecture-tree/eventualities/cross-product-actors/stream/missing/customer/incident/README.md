# stream · missing · customer · incident

**Path:** `eventualities/cross-product-actors/stream/missing/customer/incident/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone / free public core / product is the proof

## What this eventuality is
The **public process stream** is compromised by: **required artefact is absent when a public claim or release depends on it**.  
Lifecycle phase: **during or after a known incident**.  
Primary actor context: **commercial-layer customer or prospect**.

## Why it matters
Open weights or polished demos without verifiable public process stream are soft tissue.  
This path exists so the failure mode cannot hide in tribal knowledge.

## Where the bone lives
| Item | Location |
|------|----------|
| Artefacts | `models/*/stream/public_log.json` |
| Domain / policy | Domain 5 integrity |
| Verify | `hash chain tip` |
| Register on failure | registers/incidents/ |
...
1. **Stop the soft tissue:** pause any public claim that depends on healthy public process stream.
...
- BOUNDARY: commercial features cannot require opacity of public process stream if it is free-core.
