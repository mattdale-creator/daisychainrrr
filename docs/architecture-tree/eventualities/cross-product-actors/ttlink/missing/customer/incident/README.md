# ttlink · missing · customer · incident

**Path:** `eventualities/cross-product-actors/ttlink/missing/customer/incident/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone / free public core / product is the proof

## What this eventuality is
The **ttlink index / provenance links** is compromised by: **required artefact is absent when a public claim or release depends on it**.  
Lifecycle phase: **during or after a known incident**.  
Primary actor context: **commercial-layer customer or prospect**.

## Why it matters
Open weights or polished demos without verifiable ttlink index / provenance links are soft tissue.  
This path exists so the failure mode cannot hide in tribal knowledge.

## Where the bone lives
| Item | Location |
|------|----------|
| Artefacts | `models/*/ttlink/ + free_core/ttlink` |
| Domain / policy | Domain 8 free-core linking |
| Verify | `index.json + canary` |
...
1. **Stop the soft tissue:** pause any public claim that depends on healthy ttlink index / provenance links.
...
- BOUNDARY: commercial features cannot require opacity of ttlink index / provenance links if it is free-core.
