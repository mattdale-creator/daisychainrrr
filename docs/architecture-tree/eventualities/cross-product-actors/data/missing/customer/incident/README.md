# data · missing · customer · incident

**Path:** `eventualities/cross-product-actors/data/missing/customer/incident/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone / free public core / product is the proof

## What this eventuality is
The **training data / mixture** is compromised by: **required artefact is absent when a public claim or release depends on it**.  
Lifecycle phase: **during or after a known incident**.  
Primary actor context: **commercial-layer customer or prospect**.

## Why it matters
Open weights or polished demos without verifiable training data / mixture are soft tissue.  
This path exists so the failure mode cannot hide in tribal knowledge.

## Where the bone lives
| Item | Location |
|------|----------|
| Artefacts | `models/*/data/ + DATA_CARD.md` |
| Domain / policy | Domain 3 data governance |
| Verify | `data/processed/sources.json hashes` |
| Register on failure | registers/legal/ + DATA_CHANGE_LOG |
...
1. **Stop the soft tissue:** pause any public claim that depends on healthy training data / mixture.
...
- BOUNDARY: commercial features cannot require opacity of training data / mixture if it is free-core.
