# manifest · missing · investor · incident · detect

**Path:** `eventualities/cross-product-actors/manifest/missing/investor/incident/detect.md`  
**Updated:** 2026-08-08  
**Leaf type:** detect — how we notice this failure mode early

## Purpose
How we notice this failure mode early for: **manifest** · required artefact is absent when a public claim or release depends on it · phase during or after a known incident · actor funder or prospective capital.
...
3. Compare published claims (site, README, scorecard) to artefacts under `manifests/*.json + model manifests`.
4. Phase lens (**during or after a known incident**): ensure the check runs at the right gate (pre-release checklist, release seal, post-release monitor, incident, acquisition freeze).
5. Actor lens (**funder or prospective capital**): record who reported or who could conceal the issue; escalate if incentive misalignment (Domain 6 / 8).
...
## Output of detection
Write a one-paragraph incident/note with: path `cross-product-actors/manifest/missing/investor/incident/detect.md`, evidence command output, severity guess (Low–Critical per Domain 5 thresholds).
