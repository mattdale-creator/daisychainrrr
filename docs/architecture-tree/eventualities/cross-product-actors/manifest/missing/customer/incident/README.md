# manifest · missing · customer · incident

**Path:** `eventualities/cross-product-actors/manifest/missing/customer/incident/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone / free public core / product is the proof

## What this eventuality is
The **cryptographic Merkle manifests** is compromised by: **required artefact is absent when a public claim or release depends on it**.  
Lifecycle phase: **during or after a known incident**.  
Primary actor context: **commercial-layer customer or prospect**.

## Why it matters
Open weights or polished demos without verifiable cryptographic Merkle manifests are soft tissue.  
This path exists so the failure mode cannot hide in tribal knowledge.

## Where the bone lives
| Item | Location |
|------|----------|
| Artefacts | `manifests/*.json + model manifests` |
| Domain / policy | Domain 5/10 integrity |
| Verify | `ttllm-manifest verify` |
| Register on failure | registers/incidents/ + redteam |
...
2. Manual: release checklist incomplete; public URL/claim lacks linked artefact; actor (commercial-layer customer or prospect) reports inconsistency.
3. Phase-specific (during or after a known incident): pre-release CI/manual gate; release seal script; post-release monitor; incident response; acquisition due-diligence freeze.
...
1. **Stop the soft tissue:** pause any public claim that depends on healthy cryptographic Merkle manifests.
...
- BOUNDARY: commercial features cannot require opacity of cryptographic Merkle manifests if it is free-core.
