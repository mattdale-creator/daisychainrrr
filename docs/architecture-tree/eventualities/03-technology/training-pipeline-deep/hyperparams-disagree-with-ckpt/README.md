# 03-technology/training-pipeline-deep/hyperparams-disagree-with-ckpt/README.md

**Path:** `eventualities/03-technology/training-pipeline-deep/hyperparams-disagree-with-ckpt/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone / free public core / product is the proof

## What this eventuality is
The **component** is compromised by: **required artefact is absent when a public claim or release depends on it**.  
Lifecycle phase: **before public release or claim**.  
Primary actor context: **any actor (founder default)**.

## Why it matters
Open weights or polished demos without verifiable component are soft tissue.  
This path exists so the failure mode cannot hide in tribal knowledge.

## Where the bone lives
| Item | Location |
|------|----------|
| Artefacts | `repo paths` |
| Domain / policy | relevant domain |
| Verify | `verify tool` |
| Register on failure | registers/incidents/ or registers/decisions/ as appropriate |
...
1. **Stop the soft tissue:** pause any public claim that depends on healthy component.
...
- BOUNDARY: commercial features cannot require opacity of component if it is free-core.
