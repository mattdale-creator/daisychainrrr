# Domain 05 · attestation · private-when-should-be-public

**Path:** `eventualities/domain-artefact-failures/d05/attestation/private-when-should-be-public/README.md`  
**Updated:** 2026-08-08  
**Ethos:** down to the bone — org transparency equals model transparency.

## What this eventuality is
Domain **05** requires operational artefacts (founding turns 37–46).  
Here the **attestation** artefact is compromised by: **Domain transparency artefact is hidden without logged exception**.

## Why it matters
If Domain 05 artefacts are missing, stale, private-without-exception, or incomplete, the organisation can claim radical honesty while retaining soft tissue. That violates the free-core ethos.

## Normative source
- Full plan: `docs/specs/05-*.md`
- Artefact directory: `docs/specs/artefacts/05/`
- Registers: `registers/`

## Detection signals
1. Scorecard or quarterly report lists PARTIAL/TOMBSTONE without linked artefact path.
2. File missing under `docs/specs/artefacts/05/` or empty critical fields.
3. Public claim references Domain 05 compliance without log evidence.
...
## Prevention
- Release checklist requires Domain 05 row with artefact link.
